...
def byte_ecb_encrypt_hard(message, key):
    random_prefix = b'\xbd\xb5\x05G\xdcB\xe7'
...
cipher = byte_ecb_encrypt_hard(b'hi', key)
print(set1_ch7.decrypt(key, cipher))
...
def prepend_hard(unknown):
    ...
    pre = b'A' * (pad_len + extra_pad)
    return pre, ((unknown_len + pad_len) // block_size) + full_control_block
